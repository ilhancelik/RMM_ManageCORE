...
`,
  ...
  {
    id: 'proc-5',
    name: 'Update All Applications (winget)',
    description: "Uses winget command-line tool to update all upgradeable applications silently. This procedure runs as SYSTEM and primarily targets machine-wide applications; it may not update some user-specific applications. Winget (App Installer) must be installed on the target machine.",
    scriptType: 'PowerShell',
    scriptContent: `# Script to update all applications using winget.
...
if (-not $wingetPath) {
    ...
    exit 1 # Critical error, winget is required
}
...
try {
    # Execute winget upgrade command and capture all output streams
    ...
    # Winget exit codes can be complex. 0 is success.
    # A common code for "no updates found" is -1978334889 (0x8A150057) or if the output contains "No applicable update found".
    if ($exitCode -eq 0 -or $exitCode -eq -1978334889 -or $upgradeOutput -match "No applicable update found" -or $upgradeOutput -match "No installed package found matching input criteria.") {
        ...
        if ($upgradeOutput -match "No applicable update found" -or $upgradeOutput -match "No installed package found matching input criteria." -or $exitCode -eq -1978334889) {
            ...
            # Provide a summary of output if not too long, otherwise just a generic success message
            $summary = $upgradeOutput.Split([System.Environment]::NewLine) | Where-Object { $_.Trim() -ne "" }
            if ($summary.Length -gt 10) {
                 $summary = "Multiple applications updated or checked. Full details in execution log."
            } else {
                 $summary = $upgradeOutput
            }
            Write-Host "OUTPUT: Application updates attempted. Summary: $summary"
        }
    } else {
